...
if(isset($_POST['search']))
{
    ...
    $search_Query = "SELECT * FROM `arraignment pre-trial` WHERE CaseNo = $data[1]";
    ...
    if($search_Result)
    {
        if(mysqli_num_rows($search_Result))
        {
            while($row = mysqli_fetch_array($search_Result))
            {
                ...
                $DateofDetention = $row['DateofDetention'];
                $DateofArrest = $row['DateofArrest'];
                $DateofBail = $row['DateofBail'];
                $DateofVoluntarySurrender = $row['DateofVoluntarySurrender'];
                $PlaceofDetention = $row['PlaceofDetention'];
                $DateofReleased = $row['DateofReleased'];
                $ReasonforRelease = $row['ReasonforRelease'];
                $InitialSettingDate = $row['InitialSettingDate'];
                $Plea = $row['Plea'];
                $ActualArraignmentDate = $row['ActualArraignmentDate'];
                $PleaBargainDate = $row['PleaBargainDate'];
                $PreTrialDate = $row['PreTrialDate'];
                $DateRevived = $row['DateRevived'];
...
{
    $data = getPosts();
    $update_Query = "UPDATE `arraignment pre-trial` SET `AccusedStatus`='$data[2]',`DateofDetention`='$data[3]',`DateofArrest`='$data[4]',`DateofBail`='$data[5]',`DateofVoluntarySurrender`='$data[6]',`PlaceofDetention`='$data[7]',`DateofReleased`='$data[8]',`ReasonforRelease`='$data[9]',`InitialSettingDate`='$data[10]',`Plea`='$data[11]',`ActualArraignmentDate`='$data[12]',`PleaBargainDate`='$data[13]',`PreTrialDate`='$data[14]',`DateRevived`='$data[15]',`PromulgationDate`='$data[16]',`DateArchived`='$data[17]' WHERE `CaseNo` = $data[1]";
    try{
...
<section>
  <h2>Arraignment & Pre-Trial Update</h2>
  <hr>
  <form action="updatearraignment.php" method="post">
